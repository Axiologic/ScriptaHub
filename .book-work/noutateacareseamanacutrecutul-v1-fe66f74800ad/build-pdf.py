...
first,lo,hi,title_size=(7, 9, 85, 31)
...
# Native browser rendering preserves MathML layout that WeasyPrint does not implement.
math_receipt=json.loads((pub/'math/receipt.json').read_text())
assert math_receipt['sourceHtmlSha256']==hashlib.sha256(src.read_bytes()).hexdigest()
math_nodes=doc.xpath('//math');assert len(math_nodes)==len(math_receipt['formulas'])
for node,record in zip(math_nodes,math_receipt['formulas']):
 image=etree.Element('img',src=str((pub/'math'/record['file']).resolve()),alt=record['text'])
 image.set('style',f"width:{record['width']}px;height:{record['height']}px;max-width:100%;object-fit:contain")
 image.set('class','math-print');image.tail=node.tail;node.getparent().replace(node,image)
css+='.math-print{display:block;margin:5pt 0;break-inside:avoid}'
