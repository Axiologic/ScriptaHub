...
const keywordStats = Object.fromEntries(Object.entries(collection.keywords).map(([code, words]) => [code, Object.fromEntries(words.map(({ id, count }) => [id, { count }]))]));
const elements = (source) => {
  const { tags } = parseHtmlInventory(source);
  return { links: tags.filter(({ tagName }) => tagName === 'a').map(({ attrs }) => attrs), scripts: tags.filter(({ tagName }) => tagName === 'script').map(({ attrs }) => attrs) };
};
const urlFor = (href, page) => new URL(href, pathToFileURL(page));

test('Node generator reproduces every checked-in book page and gates only PDF/feedback actions', () => {
  let total = 0;
  for (const listing of collection.books) {
    ...
    for (const language of languages) {
      ...
      assert.equal(generated, readFileSync(page, 'utf8'), `${listing.id} ${language}: generated page changed`);
      ...
      for (const link of links) {
        ...
        if (target.endsWith('.pdf')) assert.equal(link['data-auth-action'], 'download', page);
        else if (target.endsWith('/feedback/index.html')) assert.equal(link['data-auth-action'], 'feedback', page);
        else assert.equal(link['data-auth-action'], undefined, `${page}: ${target} must remain public`);
      }
...
test('shared pages load authentication before their deferred interaction scripts', () => {
  for (const [relative, consumer] of [['feedback/index.html', 'workflow.js'], ['editions/index.html', 'workflow.js'], ['reader/index.html', 'reader.js']]) {
...
test('canonical readers with public PDF buttons load the shared runtime', () => {
  ...
  for (const relative of files.filter((file) => /^(?:full|short)_content\.html$/.test(path.basename(file)))) {
    ...
    const pdfs = links.map((link) => { try { return new URL(link.href ?? '', 'https://scriptahub.com/'); } catch { return null; } });
  ...
  assert.ok(checked > 0, 'canonical reader PDF coverage must not silently disappear');
});
