...
test('popup cancellation and timeout clean up listeners and timers', async () => {
  for (const cause of ['cancelled', 'timeout', 'success']) {
    ...
    const waiter = waitForCallback(host, popup, config.callback, 'state');
    if (cause === 'success') {
      ...
      timers.get(cause === 'cancelled' ? 'closed' : 'timeout')();
...
test('each action revalidates roles with UserInfo; local role flags never authorize access', async () => {
  ...
  assert.equal((await client.current()).roles[0], 'selfRegistered');
  roles = [];
