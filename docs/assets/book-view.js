  ...
  function markup(book, options = {}) {
    ...
    switch (options.variant) {
      ...
      case 'compact-context':
        return `<aside class="workflow-context workflow-book-compact" ${identity}>${cover(data)}<div>${title(data, 'h2', false)}${options.showCategory ? `<p class="category">${escape(data.category)}</p>` : ''}${options.showDescription === false ? '' : description(data)}</div></aside>`;
      default:
