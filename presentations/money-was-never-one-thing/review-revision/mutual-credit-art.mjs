...
export function mutualCreditArt(){
 ...
 label('example','Illustrative credits',600,395,22);reveal('example',2);
 ...
 add(object('trade-entries',600,455,[n('rect',{x:-320,y:-42,width:640,height:92,rx:5,fill:cream}),text('Carpenter −20',-158,-3,27,plum),text('Accountant +20',156,-3,27,teal),text('Same trade · paired entries',0,31,22,ink)],'Illustrative equal20-unit debit and credit created simultaneously; not historical transaction data.'));reveal('trade-entries',2);
 report('website-delivered',995,343,'Website');reveal('website-delivered',3);move('website-delivered',3,600,343,2500);
 add(object('next-entries',600,455,[n('rect',{x:-320,y:-42,width:640,height:92,rx:5,fill:cream}),text('−20',-215,0,30,plum),text('0',0,0,30,ink),text('+20',215,0,30,teal),text('Carpenter',-215,32,22,ink),text('Accountant',0,32,22,ink),text('Web developer',215,32,22,ink)],'A second illustrative20-unit member purchase spends the accountant credit; the network balances still sum to zero.'));hide('trade-entries',3);reveal('next-entries',3);
 label('limit','Agreed limits · default risk remains',600,546,29);reveal('limit',4);
