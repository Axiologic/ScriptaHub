...
for(const[i,s]of scenes.entries()){
 const q=stageAuthor(),{n,tint,path,text,dot,object,add,reveal,hide,move,label}=q,A='#766393',B='#d6a63e',AN='#c7b6db',BN='#efd19a';
 const chair=(id,x,y)=>{storyPerson(q,id+'-holder',x,y+95,{identity:id==='new'?'person-04-neutral':'person-02-neutral',coat:id==='new'?'#5d9c87':'#678cb5',seated:true,scale:1,chair:A});storyGesture(q,id+'-holder',2,'reflect');return id;};
 ...
 switch(i){
 case 0:chair('office',580,300);sash('rank',580,300);record('record',850,320);storyPerson(q,'reviewer',1040,445,{identity:'person-09-neutral',coat:'#c4795e',scale:.68});reveal('record',3);move('rank',4,775,425,2200);break;
 ...
 case 2:storyPerson(q,'reviewer',925,510,{identity:'person-04-neutral',coat:'#c4795e',scale:.78});storyGesture(q,'reviewer',3,'question');add(object('model',350,300,[tint(n('rect',{x:-145,y:-120,width:290,height:210,rx:18,fill:A}),AN),tint(path('M-95 20L-30 -40L35 -15L100 -65',B,12),BN),tint(path('M-75 105H75',A,18),AN)],'A model offers conditional interpretation; the line is illustrative, not reported data.'));record('human-review',755,315);label('conditional','Conditional model',350,475,31);label('judgment','Human judgment',860,555,29);break;
