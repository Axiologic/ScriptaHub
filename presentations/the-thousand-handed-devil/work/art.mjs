...
for(const[i,s]of scenes.entries()){
 ...
 switch(i){
 case 0:desk('first',275,250,'Decision');desk('second',925,250,'Review');desk('third',600,450,'Referral');appeal('appeal',275,280);move('appeal',2,925,280,1600);move('appeal',3,600,475,1600);move('appeal',4,275,280,1900);break;
 ...
 case 2:desk('owner',600,345,'Case owner');appeal('appeal',330,370);add(object('stop',960,250,[tint(n('path',{d:'M-40 -80H40L80 -40V40L40 80H-40L-80 40V-40Z',fill:B}),BN),tint(path('M-38 0H38',A,12),AN)],'Real authority can interrupt a harmful process.'));reveal('stop',2);move('appeal',3,600,370);break;
 case 3:desk('left',290,340,'Evidence');desk('right',930,340,'Remedy');appeal('appeal',290,365);move('appeal',3,930,365,2500);add(object('route',610,485,[tint(path('M-180 0H180M180 0L150 -22M180 0L150 22',B,10),BN)],'The route of responsibility reaches a usable remedy, not only another explanation.'));reveal('route',4);break;
 }
