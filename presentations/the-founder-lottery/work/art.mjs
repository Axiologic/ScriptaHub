...
for(const [i,s]of scenes.entries()){
 ...
 switch(i){
 case 0:
 person('builder',300,520,.95);bench('individual-work',600,400);tool('effort',560,285,.9);
 ...
 reveal('press',2);move('effort',3,645,285);add(object('shared-leg',600,470,[tint(path('M0 -50V50',B,16),BN)],'The constructive direction changes what supports entrepreneurial work.'));reveal('shared-leg',5);break;
 case 1:person('visible',880,430,.7);for(let j=0;j<3;j++){person('others'+j,230+j*180,520,.58);reveal('others'+j,j+2);}add(object('spotlight',880,425,[tint(n('ellipse',{cx:0,cy:70,rx:75,ry:15,fill:A}),AN)],'A bounded public stage, not a chart of probabilities.'));break;
 ...
 case 3:bench('bench',600,350);tool('tool',550,220);for(let j=0;j<3;j++){label('joint'+j,['Contribution','Reward','Exit'][j],300+j*300,525,30);reveal('joint'+j,j+2);}break;
 ...
 case 6:for(let j=0;j<3;j++){tool('duplicate'+j,260+j*330,250,.8);reveal('duplicate'+j,j+1);}bench('bench',600,425);reveal('bench',3);move('duplicate0',4,460,350);move('duplicate1',4,600,350);move('duplicate2',4,740,350);break;
 case 7:bench('bench',600,360);tool('shared',600,220);person('a',230,530,.82);person('b',980,530,.82);move('a',5,350,530);move('b',5,850,530);break;
 }
