#!/usr/bin/env node
// Migrate only completed public book films under five minutes to directed local Qwen narration.
...
async function alreadyComplete(entry){
  const [production,receipts,build]=await Promise.all([
  ...
  return production.voiceEngine==='qwen' && /Qwen3-TTS/.test(receipts?.provider||'') && audioValid && !!build && await exists(entry.shf);
}
...
function updateProject(project,title){
  ...
  return Promise.all([readJson(pfile),readJson(sfile)]).then(async([production,scenes])=>{
    ...
    for(const [sceneIndex,scene] of scenes.entries()){
      ...
      const directions=scene.lines.map((_,lineIndex)=>p.directions[Math.min(2,sceneStyle)] + (lineIndex===scene.lines.length-1?' Leave a quiet beat for the viewer to consider it.':''));
      scene.emotionalPlan={...(scene.emotionalPlan||{}),states:scene.lines.map((_,i)=>sceneIndex===0&&i===0?'curiosity':isClosing&&i===scene.lines.length-1?'invitation':'clarity'),intensities:scene.lines.map((_,i)=>Math.min(.55,p.intensities[Math.min(2,sceneStyle)]+(i===0?0:.02))),voiceDirections:directions};
    }
